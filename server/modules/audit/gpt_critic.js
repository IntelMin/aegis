...
const {criticFormatConstraint, criticZeroShotPrompt, criticFewShotPrompt} = require('./prompts');
...
function critic_response_parse(critic_outputs) {
    let output_list = [];
    for (let critic_output of critic_outputs) {
        try {
            let data = JSON.parse(critic_output);
            console.log(data);
            output_list = output_list.concat(data.output_list);
        } catch (error) {
            continue;
        }
    }
...
async function run_critic(auditor_output_list, backend, temperature, num_critic,shot = "few") {
    // backend = 'gpt-4', 
    // temperature = 0, 
    ...
    // fs.readdirSync(path.join(__dirname,"..","logs", auditor_dir)).forEach(async(filename) => {
        ...
        let critic_outputs = await gpt(critic_input, backend, temperature, num_critic);
        ...
        for (let i = 0; i < critic_bug_info_list.length; i++) {
            ...
            try {
                ...
                console.log(`len auditor: ${auditor_output_list.length} len critic ${critic_bug_info_list.length}`);
                console.log(`auditor_output_list: ${JSON.stringify(auditor_output_list)}`);
            }
        }
