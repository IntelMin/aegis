...
const { auditorPrompt, auditorFormatConstraint, topkPrompt1, topkPrompt2 } = require('./prompts'); // Adjust paths as necessary
...
function promptWrap(prompt, formatConstraint, code, topk) {
    return `${prompt}${code}${formatConstraint}${topkPrompt1.replace('{topk}', topk)}${topkPrompt2}`;
}
        ...
        try {
            ...
            if (auditorOutput && auditorOutput.output_list) {
                if (Array.isArray(auditorOutput.output_list)) {
                    ...
                    console.log("output_list is not an array:", auditorOutput.output_list);
                }
            } else {
...
async function solve(backend, temperature, topk, numAuditor, code) {
    const auditorInput = promptWrap(auditorPrompt, auditorFormatConstraint, code, topk);

    try {
        const auditorOutputs = await gpt(auditorInput, backend, temperature, numAuditor);
