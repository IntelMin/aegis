...
const bot = new TelegramBot(String(process.env.TG_BOT_TOKEN), {
  polling: true,
});
...
bot.onText(/\/subscribe/, async (msg: any) => {
  ...
  bot.sendMessage(
    chatId,
    'You have been subscribed to the bug bounty program.'
  );
});
...
bot.onText(/\/unsubscribe/, async (msg: any) => {
  ...
  bot.sendMessage(
    chatId,
    'You have been unsubscribed from the bug bounty program.'
  );
});
