...
const getFunctionsData =async(props)=>{
    ...
    const privateMatches = props.source.data.source_code.match(privateFunctionPattern);
    const payableMatches = props.source.data.source_code.match(payableFunctionPattern);
    ...
    return [internalCount,publicCount,pureCount,viewCount,payableCount,totalFunctions,privateCount]
...
const createAuditSummery = async (address) => {
  ...
  const findings = require("./cache/contracts/" + address + "/findings.json");
    const functionsData= await getFunctionsData({functions:functions,source:source});
